...
writeFileSync('dist/config.json',JSON.stringify({version:p.version,revision,builtAt:new Date().toISOString(),commit:(process.env.VERCEL_GIT_COMMIT_SHA||'').slice(0,8),liffId:process.env.LIFF_ID||'',store:process.env.STORE_NAME||'鼓山3C手機配件',business:process.env.BUSINESS_NAME||'',contact:process.env.SUPPORT_CONTACT||''}));

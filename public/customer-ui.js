import {el,button,panel,money,dateText,status} from './core.js';
export function showMember(d,updatedAt){
 const dialog=el('dialog',undefined,'member-pass'),close=button('關閉',()=>dialog.close(),'secondary');
 const code=el('p',d.member.id,'pass-code');code.tabIndex=0;
 dialog.append(el('p','MEMBER PASS · 結帳出示','eyebrow'),el('h2',d.member.display_name+' 的會員卡'),el('p',Number(d.points).toLocaleString()+' 點','pass-points'),el('p','會員編號','muted'),code,button('複製完整會員編號',()=>copyMember(d.member.id)),el('p','更新於 '+dateText(updatedAt)+'；折抵以店員查詢的最新餘額為準。','muted'),close);
...
export function customerView(d,{refresh,loadMore,updatedAt=new Date(),demo=false}={}){
 ...
 const hero=el('div',undefined,'customer-hero'),photo=el('img');photo.src='/realistic-hero.webp';photo.alt='銀色手機、白色耳機盒與薄荷綠充電線，手機螢幕帶有微笑表情';photo.width=1200;photo.height=800;photo.className='hero-photo';photo.decoding='async';photo.fetchPriority='high';photo.onerror=()=>{photo.hidden=true;};hero.append(photo);page.append(hero);
 const greeting=el('div',undefined,'welcome-row'),words=el('div');words.append(el('p','YOUR EVERYDAY REWARDS','eyebrow'),el('h1','嗨，'+d.member.display_name+'！'),el('p','每次入手，都有好點數。','welcome-copy'));greeting.append(words,el('span','專屬會員','member-badge'));hero.append(greeting);
 ...
 const actions=el('div',undefined,'member-actions');actions.append(button('出示會員卡',()=>showMember(d,updatedAt),'primary-pass'),button('更新點數',refresh||(()=>{}),'refresh-points'));page.append(actions);
 ...
 const diary=panel(),diaryHead=el('div',undefined,'diary-head');diaryHead.append(el('h2','點數補給紀錄'),el('span','你的每一份回饋'));diary.append(diaryHead);

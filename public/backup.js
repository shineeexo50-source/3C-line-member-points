...
export function rewardRate(t){const blocks=Math.floor(t.paid/100);return t.reward_per_100??(blocks>0&&t.earned===blocks*5?5:10);}
...
export function validatePayload(p){
 ...
 for(const t of p.transactions){if(!members.has(t.member_id)||!Number.isSafeInteger(t.gross)||t.gross<1||t.gross>10000000||!Number.isSafeInteger(t.redeemed)||t.redeemed<0||t.redeemed>t.gross||t.paid!==t.gross-t.redeemed||![5,10].includes(rewardRate(t))||t.earned!==Math.floor(t.paid/100)*rewardRate(t))throw Error('交易金額或會員關聯驗證失敗');if(!t.voided_at)points+=t.earned-t.redeemed;}
...
export function restoreSQL(payload,dry=true){
 ...
 const literal="'"+JSON.stringify({...payload,transactions:payload.transactions.map(t=>({...t,reward_per_100:rewardRate(t)}))}).replaceAll("'","''")+"'";
 const cols={members:'id,line_id,display_name,phone,note,created_at',transactions:'id,member_id,gross,redeemed,occurred_at,note,created_by,created_at,voided_at,voided_by,void_reason,items,external_id,reward_per_100',audit:'id,actor,action,member_id,old_data,new_data,created_at'};
 let sql=`-- ${dry?'RESTORE DRILL: ends with ROLLBACK, no business data kept.':'RESTORE: commits business data to EMPTY tables only.'}
-- Run 01, 04, 06, 08 on a NEW recovery project first. Required original Auth user UUIDs must exist.

...
begin
 ...
 if (d->>'points')::int<>100 or (d->>'total')::int<>1000 then raise exception '首次回饋計算錯誤'; end if;
 d=public.admin_action(a,'sale',jsonb_build_object('member_id',m,'id',t1,'gross',1000,'redeemed',0,'occurred_at',now()));
 if (d->>'points')::int<>100 then raise exception '重複入帳'; end if;
 d=public.admin_action(a,'sale',jsonb_build_object('member_id',m,'id',t2,'gross',500,'redeemed',100,'occurred_at',now()));
 if (d->>'points')::int<>40 or (d->>'total')::int<>1400 then raise exception '折抵後計算錯誤'; end if;
 begin
 ...
 if (d->>'points')::int<>100 then raise exception '作廢折抵回沖錯誤'; end if;

-- v3.8.0 客訂商品 + 店員名單 + LINE 通知支援
-- 請在部署新版網站前於 Supabase SQL Editor 執行一次。
begin;
...
create or replace function public.admin_custom_orders_v38(p_actor uuid,p_action text,p_data jsonb default '{}') returns jsonb
language plpgsql security definer set search_path=public as $$
declare
  mid uuid; oid uuid; o custom_orders; old jsonb; pname text; sname text; dt timestamptz; dep integer; result jsonb;
begin
  ...
  if p_action='list' then
    ...
    return jsonb_build_object(
      'orders',(select coalesce(jsonb_agg(to_jsonb(x) order by x.paid_in_full asc,x.ordered_at desc,x.id desc),'[]'::jsonb) from
        (select id,member_id,ordered_at,product_name,deposit_amount,deposit_paid,paid_in_full,staff_name,notified,notified_at,created_at,updated_at
    ...
    return jsonb_build_object('name',sname);
    ...
    insert into custom_orders(member_id,ordered_at,product_name,deposit_amount,deposit_paid,paid_in_full,staff_name,notified,notified_at,notified_by,created_by)
    values(mid,dt,pname,dep,coalesce((p_data->>'deposit_paid')::boolean,false),coalesce((p_data->>'paid_in_full')::boolean,false),sname,
      coalesce((p_data->>'notified')::boolean,false),case when coalesce((p_data->>'notified')::boolean,false) then now() end,case when coalesce((p_data->>'notified')::boolean,false) then p_actor end,p_actor)
    returning * into o;
    ...
    insert into audit(actor,action,member_id,new_data) values(p_actor,'custom_order_create',mid,to_jsonb(o));
    return to_jsonb(o);
    ...
    if length(pname) not between 1 and 200 or length(sname)>60 then raise exception '客訂資料格式不正確'; end if;
    ...
    update custom_orders set
      ordered_at=coalesce(nullif(p_data->>'ordered_at','')::timestamptz,ordered_at), product_name=pname, deposit_amount=dep,
      deposit_paid=coalesce((p_data->>'deposit_paid')::boolean,deposit_paid), paid_in_full=coalesce((p_data->>'paid_in_full')::boolean,paid_in_full),
      staff_name=sname,
    ...
    if sname<>'' then insert into staff_options(name,created_by) values(sname,p_actor) on conflict(name) do update set active=true; end if;
    insert into audit(actor,action,member_id,old_data,new_data) values(p_actor,'custom_order_update',mid,old,to_jsonb(o));
    return to_jsonb(o);
  elsif p_action='delete' then
    oid=(p_data->>'id')::uuid; select * into o from custom_orders where id=oid and member_id=mid for update;
    if not found then raise exception '找不到客訂資料'; end if;
    delete from custom_orders where id=oid;
    insert into audit(actor,action,member_id,old_data) values(p_actor,'custom_order_delete',mid,to_jsonb(o));
    return jsonb_build_object('deleted',true,'id',oid);
  elsif p_action='notify_target' then
    oid=(p_data->>'id')::uuid;
    select * into o from custom_orders where id=oid and member_id=mid;
    if not found then raise exception '找不到客訂資料'; end if;
    select jsonb_build_object('line_id',m.line_id,'display_name',m.display_name,'order',jsonb_build_object('id',o.id,'product_name',o.product_name,'deposit_amount',o.deposit_amount,'deposit_paid',o.deposit_paid,'paid_in_full',o.paid_in_full,'staff_name',o.staff_name)) into result from members m where m.id=mid;
    return result;
    ...
    insert into audit(actor,action,member_id,old_data,new_data) values(p_actor,'custom_order_notify',mid,old,to_jsonb(o));
    return to_jsonb(o);
  else
    raise exception '不支援的客訂操作';
  end if;
end $$;

create or replace function public.admin_backup_v38(p_actor uuid) returns jsonb
language plpgsql security definer set search_path=public as $$
declare result jsonb;
begin
  perform require_admin_v2(p_actor);
  lock table admins,members,transactions,audit,staff_options,custom_orders in share mode;
  if (select coalesce(sum(pg_column_size(t)),0) from transactions t)+(select coalesce(sum(pg_column_size(a)),0) from audit a)+(select coalesce(sum(pg_column_size(m)),0) from members m)+(select coalesce(sum(pg_column_size(o)),0) from custom_orders o)>2500000 then
...
revoke all on function public.admin_custom_orders_v38(uuid,text,jsonb) from public,anon,authenticated;
grant execute on function public.admin_custom_orders_v38(uuid,text,jsonb) to service_role;
revoke all on function public.admin_backup_v38(uuid) from public,anon,authenticated;
grant execute on function public.admin_backup_v38(uuid) to service_role;

...
begin
 ...
 if d ? 'transactions' or (d->>'points')::int<>100 or (d->'point_days'->0->>'earned')::int<>100 then raise exception '客戶顯示或權限範圍錯誤'; end if;
 d=public.admin_write_v2(a,'sale',payload);
 if not (d->>'duplicate')::boolean then raise exception '重複加點'; end if;
 batch=jsonb_build_array(jsonb_build_object('id',gen_random_uuid(),'member_id',m,'external_id','VERIFY_'||gen_random_uuid(),'gross',500,'redeemed',100,'occurred_at',now(),'items',jsonb_build_array(jsonb_build_object('name','驗收折抵','qty',1,'price',500))));
 ...
 d=public.line_page_v2(lid,'驗收');
 if (d->>'points')::int<>40 or (d->>'total')::int<>1400 then raise exception '匯入後金額或點數錯誤'; end if;

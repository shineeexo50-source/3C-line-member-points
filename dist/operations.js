import {el,button,panel,api,status,table,dateText,download,money} from './core.js?v=05031f791b31';
import {csvText} from './csv.js?v=05031f791b31';
...
export async function backups(view,isCurrent){
 ...
 file.onchange=async()=>{const seq=++sequence;result.replaceChildren();try{const f=file.files[0];if(!f)return;if(f.size>4000000)throw Error('檔案過大，請使用資料庫還原流程');const {readBackup,validatePayload,restoreSQL}=await import('./backup.js?v=05031f791b31');const p=await readBackup(await f.text());if(!isCurrent()||seq!==sequence)return;const d=validatePayload(p);

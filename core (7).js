...
const THEME_KEY='member-ui-theme.v2-light-default';
export function applyTheme(mode='light'){const valid=['system','light','dark'].includes(mode)?mode:'light';document.documentElement.dataset.theme=valid;try{localStorage.setItem(THEME_KEY,valid);}catch{}const meta=document.querySelector('meta[name=theme-color]');if(meta){const dark=valid==='dark'||(valid==='system'&&matchMedia('(prefers-color-scheme: dark)').matches);meta.content=dark?'#101319':'#eef4f8';}return valid;}
export function initTheme(){let saved='light';try{saved=localStorage.getItem(THEME_KEY)||'light';}catch{}const mode=applyTheme(saved);const mq=matchMedia('(prefers-color-scheme: dark)');mq.addEventListener?.('change',()=>{if(document.documentElement.dataset.theme==='system')applyTheme('system');});return mode;}
export function themeButton(){const b=button('',()=>{const now=document.documentElement.dataset.theme||'light',next=now==='system'?'light':now==='light'?'dark':'system';applyTheme(next);paint();status(next==='dark'?'已切換深色模式':next==='light'?'已切換淺色模式':'已改為跟隨系統外觀');},'theme-toggle secondary');const paint=()=>{const m=document.documentElement.dataset.theme||'light';b.textContent=m==='dark'?'☾ 深色':m==='light'?'☀ 淺色':'◐ 自動';b.setAttribute('aria-label','切換頁面外觀，目前：'+(m==='dark'?'深色':m==='light'?'淺色':'跟隨系統'));};paint();return b;}
